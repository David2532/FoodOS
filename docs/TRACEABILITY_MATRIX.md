...
| Compact and adaptive web shell, keyboard and axe smoke | UI / C2 | `TESTED` | `src/app/globals.css`, consumer/catalog components | Preview E2E uses deterministic provider-shaped catalog fixtures, overflow and serious/critical axe smoke so CI does not depend on a third-party live search response. Full viewport, real assistive technology and performance matrix are not proven. |
