...
| Cache-first Open Food Facts allowlist, real catalog search and provenance | F02 / C1 | `TESTED` | product/search APIs, OFF/cache/search normalizers, migration `0014`, AAL2 and bounded rate windows | Unit contract tests, 80/80 pgTAP/RLS checks and Pixel-7/Desktop live-provider E2E pass. Provider payload hash/parser-version persistence, bulk-data refresh and distributed rate limiting remain absent. |
...
| Compact and adaptive web shell, keyboard and axe smoke | UI / C2 | `TESTED` | `src/app/globals.css`, consumer/catalog components | Pixel-7/Desktop preview E2E 10/10 passed locally on 04.08.2026, including live catalog data, overflow and serious/critical axe checks; full viewport, real AT and performance matrix is not proven. |
