...
Run `npm run agent:validate-company` after changing the registry or governance. The command rejects required-role removal, legacy super-agent identifiers, self-approval, weakened C0/C1 protection, incomplete assurance and missing forbidden autonomous actions. It also runs at the start of `npm run verify`.
...
A product-plan change that increases capture burden, introduces a universal health score, weakens the useful free core or moves internal dashboards ahead of the trusted food loop requires explicit CPO review and must record the user-evidence rationale.
