...
| Product catalog/cache | GTIN, labels, nutrition, ingredients, source metadata/image URL | provider licence and legitimate interest after review | provider refresh/correction policy; public catalog generations retain only approved operational evidence | OFF license boundary/provenance; no household FK; no managed bulk import as of 2026-08-04 |
| Public catalog import operations | source dataset URL/schema/retrieval time, generation, aggregate counts, normalized hash and safe failure code | security/operations plus provider-licence review | staging/active/superseded/failed generation under documented recovery and retention policy | server-only service role; no raw provider objects, private household data or product payload logs |
